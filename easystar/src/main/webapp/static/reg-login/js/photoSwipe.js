...
for(let i in vm.item){
        vm.item[i].page = (vm.item.length - i);
    }
var photoSwipe={
    /*用户信息数组*/
    imgArr:[],
    ...
    animateMove:function (el,val) {
        ...
        var moveTime=setTimeout(function () {
            if(photoSwipe.index > vm.item.length){
                photoSwipe.index = 1;
            }
            if(photoSwipe.index === 1){
                photoSwipe.index = 2;
            }
            vm.add+=1;
            vm.item.unshift(vm.item[(vm.item.length - 1)])
            vm.item.pop();
            var ind_el=$(".item-"+(photoSwipe.index - 1));
...
$("#photo_box").on("touchstart",function(e) {
    ...
    // var act_el=$(".item-"+(photoSwipe.index-1).toString(10));
});
